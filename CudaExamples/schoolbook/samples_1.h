...
// 12 NESTED HELLOWORLD
/*
    having helloworld from nested kernels (kernels invoked from other kernels of GPU)
*/
...
void nestedHelloWorld() {
    ...
    // TODO: eventual implementation of inputs 
    /*if (argc > 1)
    {
        igrid = atoi(argv[1]);
...
// 13 NESTED REDUCE 
/*
    example of nested reduction done with multiple kernels invocation from other kernels 
    on GPU 
*/
...
void nestedReduce2() {
    ...
    CHECK(cudaGetDeviceProperties(&deviceProp, dev));
    printf("starting reduction at ");
    printf("deive %d: %s", dev, deviceProp.name);
    CHECK(cudaSetDevice(dev));
    ...
    // set up execution configuration 
    int nblock = 2048;
    int nthread = 512; // intial block size 
    ...
    dim3 block(nthread, 1);
    dim3 grid((size + block.x - 1) / block.x, 1);
    printf("array %d grid %d block %d", size, grid.x, block.x);
    ...
    printf("gpu nestedNoSyn\t\telapsed %f sec gpu_sum: %d <<<grid %d block %d>>>\n", iElaps, gpu_sum, grid.x, block.x);
